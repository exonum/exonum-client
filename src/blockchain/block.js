...
export function verifyBlock (data, validators) {
  return new Promise(resolve => {
    pbjs.main(['--target', 'json', '../../proto/protocol.proto'], function (err, output) {
      if (err) {
        throw err
      }

      const jsonDescriptor = JSON.parse(output)
      const root = Protobuf.Root.fromJSON(jsonDescriptor)
      const Block = root.lookupType('Block')
      const Precommit = root.lookupType('Precommit')
      ...
      for (let i = 0; i < data.precommits.length; i++) {
        ...
        const time = nano.fromISOString(precommit.payload.time)
        payload.time = {
          seconds: time[0],
          nanos: time[1]
        }
